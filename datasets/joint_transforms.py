...
class RandomCrop(object):
    def __init__(self, size, ignore_index, nopad=True):
    ...
    def __call__(self, img, mask, centroid=None):
        ...
        # 默认走这条路
            ...
            if pad_h or pad_w:
                ...
                # fill: 这定义了要应用的像素填充值或颜色值。默认值为0，表示颜色为黑色。填充的是边界
                mask = ImageOps.expand(mask, border=border, fill=self.ignore_index)
                w, h = img.size

        if centroid is not None:
            # 需要确保质心被裁剪覆盖，并且裁剪完全位于图像内
            c_x, c_y = centroid
            max_x = w - tw
            max_y = h - th
...
class RandomSizeAndCrop(object):
    def __init__(self, size, crop_nopad,scale_min=0.5, scale_max=2.0, pre_size=None):
        self.size = size
        #fill: 这定义了要应用的像素填充值或颜色值。默认值为0，表示颜色为黑色。crop_nopad传的是false
        self.crop = RandomCrop(self.size, ignore_index=0,nopad=crop_nopad)
