...
def _segm_hrnet(name, backbone_name, num_classes, pretrained_backbone):
    if hrnetv2 is None:
        raise ImportError("HRNet backbone sources are not available in baseline/DeepLab/DeepLab")
...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    if backbone_name == 'resnet50':
        ...
        raise NotImplementedError(f"Unsupported ResNet backbone: {backbone_name}")
...
def _segm_xception(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if xception is None:
        raise ImportError("Xception backbone sources are not available in baseline/DeepLab/DeepLab")
    if output_stride==8:
...
def _segm_mobilenet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if mobilenetv2 is None:
        raise ImportError("MobileNetV2 backbone sources are not available in baseline/DeepLab/DeepLab")
    if output_stride==8:
        aspp_dilate = [12, 24, 36]
    else:
        aspp_dilate = [6, 12, 18]
