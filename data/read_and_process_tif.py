...
def sample_analysis(filepath, samples=5):
    ...
    with rasterio.open(filepath) as src:
        ...
        for i in range(samples):
            # 随机选择一个小窗口
            win_height = min(1000, height//10)  # 窗口高度不超过原图的1/10
...
def split_and_save_tiles(image_path, label_path, output_dir, tile_size=256, overlap=0):
    """
    将大图像分割成小块并保存为PNG格式
    """
    print(f"\n=== 开始分割图像为 {tile_size}x{tile_size} 的块 ===")
    ...
    with rasterio.open(image_path) as img_src, rasterio.open(label_path) as lbl_src:
        ...
        print(f"需要分割成 {n_tiles_x} x {n_tiles_y} = {n_tiles_x * n_tiles_y} 个小块")
        ...
        for y in range(n_tiles_y):
            for x in range(n_tiles_x):
                ...
                # 处理标签 - 根据代码中使用 np.max(mask, axis=2) 推断，标签可能是彩色标签
                # 如果标签已经是整数值，直接保存
                label_pil = Image.fromarray(label_tile.astype('uint8'), mode='L')  # 使用灰度模式

                # 决定这是训练集还是验证集（这里简单地按比例分配）
                is_val = (train_count + val_count) % 10 == 0  # 大约10%的数据作为验证集

                # 生成文件名
                filename_base = f"tile_{y:04d}_{x:04d}"
                ...
                if (train_count + val_count) % 50 == 0:
                    print(f"已处理 {train_count + val_count} 个瓦片... (训练: {train_count}, 验证: {val_count})")

        print(f"分割完成! 总共 {train_count + val_count} 个瓦片 (训练: {train_count}, 验证: {val_count})")
        print(f"输出保存至: {output_dir}")


def main():
    parser = argparse.ArgumentParser(description="分析和处理遥感TIFF图像")
